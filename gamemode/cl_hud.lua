surface.CreateFont("HudSwing", {font = "Trebuchet18", size = 72})
surface.CreateFont("HudHole", {font = "Trebuchet18", size = 46})
surface.CreateFont("HudTitle", {font = "Trebuchet18", size = 32})
surface.CreateFont("HudMessage", {font = "Trebuchet18", size = ScreenScale(30)})
surface.CreateFont("HudNormal", {font = "Trebuchet18", size = ScreenScale(20)})
...
function GM:HUDPaint()
	...
	if self:GetState() == STATE_WAITING then
		local hue = (SysTime()) * 50 % 360

		-- Convert HSV to RGB
		local textColor = HSVToColor(hue, 1, 1)
	
		-- Draw the text using draw.SimpleText
		draw.SimpleText("Framework | Made by Foksiu", "Trebuchet18", 0, 0, textColor)
		draw.SimpleText("Type framework_menu into console to get started", "Trebuchet18", 0, 13, Color(255,255,255))
	end
	draw.SimpleText("Time left: " .. GAMEMODE:GetTimeLeft(), "Trebuchet18", 0, ScrH() / 1 - 20, Color(255,255,255))
end
...
function GM:DrawHUDPower()
	...
	surface.SetDrawColor(120, 120, 80, 150)
	surface.DrawRect(offset, ScrH() - offset - h - y, w, h, thickness, percent)

	if Swing.lastpower > 0 then
		surface.SetDrawColor(255, 0, 0, 255)
		surface.DrawRect((offset + (w * lastPowerPercent)), ScrH() - offset - h - y + thickness, 2, h - (thickness * 2), Color(255,0,0))
	end
end
