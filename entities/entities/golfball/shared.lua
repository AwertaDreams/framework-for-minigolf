...
function ENT:Pocket()
	...
	--[[for i, ply in ipairs(player.GetAll()) do
		ply:ChatPrint(owner .. " scored " .. owner:Swing())
	end
	]]
	self:GetOwner():ChatPrint(owner:Swing())
...
function ENT:HandleReady( vel )
	if vel < 2 and self:HitWorld( self:GetDownTrace() ) then
		if not self.ReadyDelay then
...
function ENT:DrawName( owner )
	...
	cam.Start3D2D( pos, Angle( 0, ang.y, 0 ), SinBetween( 0.12, 0.15, RealTime() * 1 ) )
		...
		draw.SimpleTextOutlined( string.upper( owner:GetName() ), "Trebuchet24", 25, -30, Color( self.Color.r, self.Color.g, self.Color.b, opacity ), 0, 0, 2, Color( 0, 0, 0, 255 ) )
	cam.End3D2D()
